...
def hr_at_k(actual, predicted, topk):
    n_users = len(actual)
    r = predicted[0:n_users, 0:topk]
    sum = np.sum(r, axis=1).squeeze()
    count = np.count_nonzero(sum)
    return count
...
def test_one_batch(X, topks):
    ...
    recall, ndcg, hr, precision = [], [], [], []
    for k in topks:
        recall.append(Recall_ATk(groundTrue, r, k))
        ndcg.append(NDCGatK_r(groundTrue, r, k))
        hr.append(hr_at_k(groundTrue, r, k))
        precision.append(precision_at_k(groundTrue, r, k))
...
def eval_PyTorch(model, data_generator, Ks):
    result = {'recall': np.zeros(len(Ks)), 'ndcg': np.zeros(len(Ks)), 'hr': np.zeros(len(Ks)), 'precision': np.zeros(len(Ks))}
    ...
    for batch_result in batch_results:
        ...
        result['hr'] += batch_result['hr'] / n_test_users
